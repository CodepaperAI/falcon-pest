...
export async function POST(request) {
  try {
    ...
    const { name, email, phone, service, date, note } = parsed.data;
    ...
    const { error } = await resend.emails.send({
      ...
      html: bookingEmailHtml({ name, email, phone, service, date, note }),
    });
...
function bookingEmailHtml({ name, email, phone, service, date, note }) {
  return `<!DOCTYPE html>
<html>
